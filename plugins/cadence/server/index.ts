...
server.registerTool(
  ...
  {
    description:
      "List all files currently uploaded to the Gemini Files API for this API key. Useful for debugging what's cached, finding orphaned uploads, or checking file state/expiry. Files auto-expire after 48 hours.",
    inputSchema: {},
...
server.registerTool(
  ...
  {
    description:
      "Delete every file in the Gemini Files API that is NOT referenced by our local disk cache. Orphans are left over from old uploads before the cache existed or from source files that have since changed. Use dryRun: true first to preview what would be deleted.",
    inputSchema: {
      dryRun: z
        .boolean()
        .default(true)
  ...
  "reason-find-action-moments",
  {
    ...
    inputSchema: {
      ...
      fps: z
        ...
        .optional()
        .describe(
          "Frames per second to sample from the video. Default 5 (good for action). Use 1-2 for static/slow footage, 10+ for very fast action. Cost scales linearly."
        ),
    },
    annotations: { readOnlyHint: true, openWorldHint: true },
  },
...
server.registerTool(
  ...
  {
    description:
      "Ask Gemini to listen to audio and choose the best high-energy segment for a montage. The choice snaps to a beat grid you provide (pairs well with audio-detect-beats).",
    inputSchema: {
      audioPath: z.string().describe("Absolute path to the audio file"),
  ...
  async ({ audioPath, beats, targetDurationS }) =>
    json(await reason.pickEnergySegment(audioPath, beats, targetDurationS))
);
  ...
  "reason-plan-edit",
  {
    ...
    inputSchema: {
      ...
      stylePrompt: z
        ...
        .optional()
        .describe(
          "Optional creative direction for the edit style (cross-cutting rules, pacing, aesthetic references). Prepended to the default planner prompt."
        ),
    },
    annotations: { readOnlyHint: true, openWorldHint: true },
  },
  async ({ beats, segmentStartS, segmentEndS, moments, stylePrompt }) =>
    json(await reason.planEdit(beats, segmentStartS, segmentEndS, moments, stylePrompt))
);
...
server.registerTool(
  ...
  {
    description:
      "Add a video clip to the timeline at a specific output-timeline position. videoEndS must be strictly greater than videoStartS, and videoStartS/positionS must be non-negative. The call is rejected with an error if these invariants are violated.",
    inputSchema: {
...
server.registerTool(
  ...
  {
    description:
      "Render the current timeline to an MP4 file using ffmpeg. Requires at least one clip and an audio track.",
    inputSchema: {
  ...
  async ({ outputPath }) => {
    ...
    if (state.clips.length === 0) return err("Timeline has no clips. Add clips with timeline-add-clip first.");
    if (!state.audio) return err("Timeline has no audio track. Set one with timeline-set-audio first.");
