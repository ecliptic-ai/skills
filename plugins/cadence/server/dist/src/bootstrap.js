...
export async function bootstrap() {
    if (!process.env.GEMINI_API_KEY?.trim()) {
        fail("GEMINI_API_KEY is not set. Open /plugin → Installed → cadence → " +
    ...
    if (!pythonBin || !beatsScript || !cacheDir)
        return;
    ...
    const cachedContents = existsSync(requirementsCached)
        ...
        : null;
    if (existsSync(pythonBin) && srcContents === cachedContents)
        return;
    ...
    try {
        ...
        if (existsSync(requirementsCached))
            rmSync(requirementsCached);
