...
export function client() {
    if (!cachedClient) {
        ...
        if (!apiKey) {
            throw new Error("Gemini API key is not configured. Open /plugin → Installed → cadence → Configure options to set gemini_api_key, then /reload-plugins.");
        }
...
export function detectMimeType(path, kind) {
...
export async function uploadAndWait(path, mimeType) {
    const ai = client();
