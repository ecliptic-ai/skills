...
function client() {
    if (!cachedClient) {
        ...
        if (!apiKey) {
            throw new Error("Gemini API key is not configured. Open Claude Code's plugin browser (`/plugin`), select 'cadence', choose 'Configure options', and paste your key (get one at https://aistudio.google.com/apikey).");
        }
...
function detectMimeType(path, kind) {
    const ext = path.toLowerCase().split(".").pop() ?? "";
    const map = kind === "video" ? VIDEO_MIMES : AUDIO_MIMES;
    const found = map[ext];
    if (!found) {
        throw new Error(`Unsupported ${kind} extension: .${ext}. Supported: ${Object.keys(map).join(", ")}`);
    }
    return found;
}
...
async function logGeminiCall(tool, prompt, extracted, result) {
...
export async function purgeOrphans(dryRun) {
    const cache = await loadCache();
...
async function uploadAndWait(path, mimeType) {
    const ai = client();
...
export async function findActionMoments(videoPath, fps = 5) {
...
Spread moments across the whole video. Return them sorted by intensity (highest first).`;
    ...
    const moments = extracted.moments.map((m) => ({
...
export async function pickEnergySegment(audioPath, beats, targetDurationS) {
...
export async function planEdit(beats, segmentStartS, segmentEndS, moments, stylePrompt) {
    const ai = client();
...
- All timestamps in MM:SS.mmm format, copy beat values verbatim when they appear`;
    ...
    const extracted = extractResponse(response);
    const clips = extracted.clips.map((c) => ({
