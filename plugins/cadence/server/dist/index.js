...
// ---------- Session lifecycle ----------
server.registerTool("session-begin", {
    description: "Begin a new Gemini edit session for this montage. Uploads the video and audio to the Gemini Files API, creates an explicit cache with a system instruction summarizing the user's creative intent, and opens a conversation Gemini will carry across subsequent reason-* calls. Must be called before any reason-* tool. Ends any prior session first.",
    inputSchema: {
        ...
        userIntent: z
            .string()
            .describe("The user's creative direction for this edit, in their own words or your synthesis. Examples: 'fast-paced hand-to-hand combat highlights for an anime action montage', 'slow cinematic showcase of the protagonist's powers'. This becomes Gemini's primary scoring axis throughout the session."),
    },
...
server.registerTool("session-end", {
...
server.registerTool("gemini-list-files", {
    description: "List all files currently uploaded to the Gemini Files API for this API key. Files auto-expire after 48 hours.",
    inputSchema: {},
...
server.registerTool("gemini-purge-orphans", {
    description: "Delete every file in the Gemini Files API that is NOT referenced by our local disk cache. Use dryRun: true first to preview.",
    inputSchema: {
        dryRun: z
            .boolean()
            .default(true)
...
server.registerTool("reason-find-action-moments", {
    description: "Ask Gemini to watch the session's video and return every moment that fits the user's intent. Requires an active session (see session-begin). Stores the full moments list in the session's artifacts; returns a compact summary (count + highest intensity).",
    inputSchema: {
        fps: z
            .number()
            .min(0.1)
            .max(60)
            .optional()
            .describe("Frames per second to sample. Default 5 (good for action). Use 1-2 for slow footage, 10+ for very fast action. Cost scales linearly."),
        focusPrompt: z
            .string()
            .optional()
            .describe("Optional additional narrowing for this step. The session's userIntent already steers selection; use this only to refine (e.g. 'exclude moments shorter than 0.5s', 'prefer character-driven frames over environmental shots')."),
    },
    annotations: { readOnlyHint: false, openWorldHint: true },
}, async ({ fps, focusPrompt }) => json(await reason.findActionMoments(fps, focusPrompt)));
server.registerTool("reason-pick-energy-segment", {
    description: "Ask Gemini to listen to the session's audio and pick the best segment for the montage, snapped to the provided beat grid. Requires an active session. Stores the segment in the session's artifacts; returns the picked bounds and reasoning.",
    inputSchema: {
        beats: z
            .array(z.number())
        ...
        focusPrompt: z
            .string()
            .optional()
            .describe("Optional structural preference (e.g. 'prefer the drop after the build; avoid intro/outro'). Skip unless the user has a specific preference."),
    },
    annotations: { readOnlyHint: false, openWorldHint: true },
}, async ({ beats, targetDurationS, focusPrompt }) => json(await reason.pickEnergySegment(beats, targetDurationS, focusPrompt)));
server.registerTool("reason-plan-edit", {
    ...
    inputSchema: {
        ...
        stylePrompt: z
            .string()
            .optional()
            .describe("Arrangement direction — how clips should be cut and paced (e.g. 'fast anime cross-cutting, rarely hold a shot', 'slow cinematic holds with cuts on downbeats'). Applied after intent filtering."),
    },
    annotations: { readOnlyHint: false, openWorldHint: true },
}, async ({ beats, stylePrompt }) => json(await reason.planEdit(beats, stylePrompt)));
// ---------- Timeline inspection + conflict resolution ----------
server.registerTool("timeline-list-issues", {
    description: "Return every validation issue on the current timeline (overlaps, gaps, anchor drift, out-of-segment). Each issue includes affected clip IDs and a human-readable message. Call timeline-inspect-clip for full clip metadata.",
    inputSchema: {},
    annotations: { readOnlyHint: true, openWorldHint: false },
}, async () => json({ issues: timeline.listIssues() }));
server.registerTool("timeline-inspect-clip", {
    description: "Return the full clip document for a given clip ID: source range, position, anchor metadata, and Gemini's description + reasoning.",
    inputSchema: {
...
server.registerTool("timeline-update-clip", {
    description: "Patch a clip's source range, position, anchor, description, or reasoning. Use this to resolve validation issues (e.g. trim a clip's source end to remove an overlap with the next clip). Revalidates the timeline after the change.",
    inputSchema: {
...
server.registerTool("timeline-remove-clip", {
...
server.registerTool("timeline-insert-clip", {
    description: "Insert a new clip into the timeline (e.g. to fill a gap identified by timeline-list-issues). Revalidates the timeline.",
    inputSchema: {
    ...
    try {
        const clip = timeline.insertClip({ sourcePath, sourceStartS, sourceEndS, positionS, description, reasoning }, "manual");
...
server.registerTool("timeline-add-clip", {
    description: "Add a video clip to the timeline at a specific output-timeline position. Use this for manual composition; in the session flow, reason-plan-edit applies clips for you.",
    inputSchema: {
        sourceUri: z.string(),
...
server.registerTool("render-final", {
    description: "Render the current timeline to an MP4 via ffmpeg. Refuses if the timeline has error-severity issues (overlaps, out-of-bounds). Call timeline-list-issues first if unsure.",
    inputSchema: {
    ...
    if (state.clips.length === 0)
        return err("Timeline has no clips. Apply a plan or add clips first.");
    if (!state.audio)
        return err("Timeline has no audio track. Call reason-plan-edit or timeline-set-audio first.");
    if (timeline.hasErrors()) {
        const errors = state.issues.filter((i) => i.severity === "error");
        return err(`Timeline has ${errors.length} error-severity issue(s); refuse to render. Call timeline-list-issues and resolve them (via timeline-update-clip / timeline-remove-clip) before retrying.`);
    }
